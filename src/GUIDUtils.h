...
namespace TGUIDInternal
{
	inline constexpr std::string_view anonIdPrefix = "anon-";
}
...
public:
	...
	{
		std::string name = fmt::format("{}{}", TGUIDInternal::anonIdPrefix, ++sAutoIncrement);
...
private:
	...
	static void Validate(std::string_view name, bool isAutoGen)
	{
		...
		if (!isAutoGen)
		{
			auto res = std::mismatch(TGUIDInternal::anonIdPrefix.begin(), TGUIDInternal::anonIdPrefix.end(), name.begin());

			if (res.first == TGUIDInternal::anonIdPrefix.end())
			{
